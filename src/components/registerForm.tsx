...
export default function RegisterForm() {
  ...
  const Item = ({ icon = "", label = "label" }: { icon: string; label: string }) => {
    return (
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await axios.post('https://api-builder-production.up.railway.app/users/v1/register', userData).then((response) => {
